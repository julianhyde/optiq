  ...
  @Test public void testJoinFiveWay() {
    OptiqAssert.that()
        ...
        .query(
            ...
            + " \"product_class\".\"product_family\"")
        .returnsUnordered(
            "c0=USA; c1=1997; c2=Non-Consumable; m0=16414",
...
//      if (query.i != FOODMART_QUERIES.size() - 1) continue;
  ...
  /** Query 119 is difficult to plan efficiently. As of optiq-0.6, the planning
   * process generated 42,000 instances of RexProgram. */
  @Test public void testFoodMartQuery119() throws IOException {
    final FoodmartTest.FoodMartQuerySet set =
        FoodmartTest.FoodMartQuerySet.instance();
    OptiqAssert.that()
        .with(OptiqAssert.Config.FOODMART_CLONE)
        .query(set.queries.get(119).sql)
        .returnsCount(set.queries.get(119).rows.size());
  }
